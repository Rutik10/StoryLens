...
function StepIcon({ icon }) {
  ...
  if (icon === "dash") return <div className="w-3 h-0.5 bg-zinc-600 rounded" />;
  return <div className="w-2 h-2 rounded-full bg-zinc-700" />;
}

export default function Pipeline({ steps, stepStatus }) {
    ...
    <div className="flex flex-col items-center gap-10">
      <div className="text-center space-y-2">
        <h2 className="text-3xl font-bold">
          {currentStep ? currentStep.label.replace("...", dots) : "Processing your story"}
        </h2>
        ...
        {steps.map((step) => {
          ...
          return (
            ...
            >
              ...
              <p className={`text-sm font-medium ${config.color}`}>{step.label}</p>
            </div>
          );
        })}
      </div>
      ...
      <p className="text-zinc-600 text-xs text-center max-w-xs">
        Veo 3.1 generates an 8-second video. This typically takes 1–2 minutes.
      </p>
    </div>
  );
}
